...
#define SMOKE_TIME	8.0f;

const float GRENADE_COEFFICIENT_OF_RESTITUTION = 0.2f;

#define GRENADE_MODEL "models/Weapons/w_grenade.mdl"

class CGrenadeSmk : public CBaseGrenade
{
	DECLARE_CLASS( CGrenadeSmk, CBaseGrenade );
...
LINK_ENTITY_TO_CLASS( grenade_smk, CGrenadeSmk );

BEGIN_DATADESC( CGrenadeSmk )
...
// Purpose: 
//-----------------------------------------------------------------------------
CGrenadeSmk::~CGrenadeSmk( void ) {}

void CGrenadeSmk::Detonate( void ) {
	trace_t		tr;
	Vector		vecSpot;// trace starts here!

	SetThink( &CGrenadeSmk::SpewThink );
...
void CGrenadeSmk::Explode( trace_t *pTrace, int bitsDamageType ) {

	//
	// In this case, start spewing smoke
	...
	m_flSpewEndTime = gpGlobals->curtime + SMOKE_TIME;

	DispatchParticleEffect("smk_gren", GetAbsOrigin() + Vector(0,0,10), GetAbsAngles(), this);
}

void CGrenadeSmk::Spawn( void )
{
	Precache( );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeSmk::OnRestore( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeSmk::CreateEffects( void )
{
	...
	if ( m_pMainGlow != NULL )
	{
		...
		m_pMainGlow->SetTransparency( kRenderGlow, 255, 255, 255, 0, kRenderFxNoDissipation );
	...
	if ( m_pGlowTrail != NULL )
	{
		...
		m_pGlowTrail->SetTransparency( kRenderTransAdd, 255, 0, 0, 0, kRenderFxNone );
...
bool CGrenadeSmk::CreateVPhysics()
{
...
void CGrenadeSmk::VPhysicsUpdate( IPhysicsObject *pPhysics )
{
...
void CGrenadeSmk::Precache( void )
{
	...
	PrecacheParticleSystem("smk_gren");

	BaseClass::Precache();
}

void CGrenadeSmk::SetTimer( float detonateDelay, float warnDelay )
{
	m_flDetonateTime = gpGlobals->curtime + detonateDelay;
	m_flWarnAITime = gpGlobals->curtime + warnDelay;
	SetThink( &CGrenadeSmk::DelayThink );
	SetNextThink( gpGlobals->curtime );

	CreateEffects();
}

void CGrenadeSmk::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
...
void CGrenadeSmk::SpewThink(void) {
...
void CGrenadeSmk::SetVelocity( const Vector &velocity, const AngularImpulse &angVelocity )
{
...
int CGrenadeSmk::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
...
bool CGrenadeSmk::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
void CGrenadeSmk::InputSetTimer( inputdata_t &inputdata )
{
...
CBaseGrenade *SmkGren_Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer, bool combineSpawned )
{
	...
	CGrenadeSmk *pGrenade = (CGrenadeSmk *)CBaseEntity::Create( "grenade_smk", position, angles, pOwner );
...
{
	const CGrenadeSmk *pSmk = dynamic_cast<const CGrenadeSmk *>( pEntity );
	if ( pSmk ) {
		return pSmk->WasPunted();
	}
